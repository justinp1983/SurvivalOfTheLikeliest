...
%TO DO:
% add in "this result was previously shown..."
% fix equation references
% update language: fitness, natural selection, etc.
% find better places to introduce abbreviation
% make exp vs e^ consistent
% move all neg signs in front of fractions
% does growth rate of resistant clones depend on treatment?
% fix the freaking beta terms :/
...
% some kind of graphic here? maybe showing the mutational pathways
...
% Figure out how to make this a caption
...
& \frac{C_4}{\beta_B(\beta_A + \beta_B e^{-r_S \Delta T})}\Big[1 - \exp\Big(-\frac{e^{r_S' (T+\Delta T)}}{r_S'} (\beta_A - \beta_B e^{-r_S' \Delta T}) \Big)\Big]- \\
& \frac{C_4}{\beta_A \beta_B} \exp(-\frac{\beta_B}{r_S'} e^{r_S' T}) \Big[1 - \exp(-\frac{\beta_A}{r_S} e^{r_S' (T+\Delta T)})\Big]
% \frac{C_1}{\alpha_B} \frac{1}{\alpha_A + \alpha_B e^{-r_S \Delta T}} [\exp(-\frac{1}{r_S} (\alpha_A + \alpha_B e^{-r_S \Delta T}) - \exp(-\frac{1}{r_S} (\alpha_A + \alpha_B e^{-r_S \Delta T}) e^{r_S \Delta T})] + \\
%&\frac{C_1}{\alpha_B} e^{-\alpha_B/r_S} [\exp(-\frac{\alpha_A}{r_S} e^{r_S T}) - \exp(-\frac{\alpha_A}{r_S} e^{r_S \Delta T})] + \\
%& K \sum_{k=0}^{\infty} \Bigg\{\frac{(-\beta_A/r_S')^k}{k!} (\frac{r_S}{\alpha_B}e^{r_S \Delta T})^{k r_S'/r_S} \cdot \\
%& \Big[\gamma\big(\frac{r_S'}{r_S}(k+1),\frac{\alpha_B}{r_S} e^{r_S T}\big) - \gamma\big(\frac{r_S'}{r_S}(k+1),\frac{\alpha_B}{r_S} e^{r_S (T-\Delta T)}\big)\Big]\Bigg\} + \\
%& \frac{C_3}{\beta_A \alpha_B} [1-\exp(-\frac{\beta_A}{r_S'} e^{r_S' (T+\Delta T)})]\cdot [\exp(-\frac{\alpha_B}{r_S} e^{r_S T}) - \exp(-\frac{\alpha_B}{r_S})] + \\
%& \frac{C_4}{\beta_B} \frac{1}{\beta_A + \beta_B e^{-r_S \Delta T}}[1 - \exp(-\frac{\beta_A - \beta_B e^{-r_S' \Delta T}}{r_S'} e^{r_S' (T+\Delta T)}]+ \\
%&-\frac{C_4}{\beta_A \beta_B} \exp(-\frac{\beta_B}{r_S'} e^{r_S' T}) [1 - \exp(-\frac{\beta_A}{r_S} e^{r_S' (T+\Delta T)})]
\end{split}
\end{equation}
